        ...
        <aside className={`bg-white border-r pt-20 border-gray-200 text-gray-800 w-64 fixed inset-y-0 left-0 z-30 transform transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
            ...
            <nav className='h-full flex flex-col justify-between'>
               <div className='px-4 py-6 space-x-8'>
                {
                    sidebarItems.map((item)=>(
                        <Link href={item.href} key={item.href} passHref>
                            ...
                            <Button variant={pathname === item.href ? 'secondary' : 'ghost'} className={`w-full justify-start py-3 ${pathname === item.href ? 
               ...
               <div className='p-4 border-t border-gray-200'>
                ...
                <Button variant={pathname === "/settings" ? 'secondary' : 'outline'} className={`w-full justify-start py-3 ${pathname === "/settings" ? 
                                'bg-green-100 text-green-800' : 
                                "text-gray-600 hover:bg-gray-100"}`}>
